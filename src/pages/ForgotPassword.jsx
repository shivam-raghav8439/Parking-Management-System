...
const ForgotPassword = () => {
  ...
  // SMS OTP fields & Wizard Step
  const [otpStep, setOtpStep] = useState(1); // 1: Enter Mobile, 2: Enter OTP, 3: Enter New Password
  ...
  const handleEmailSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await client.post('/auth/forgot-password/email', { email });
      setSentEmail(true);
  ...
  const handleSendOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await client.post('/auth/forgot-password/otp', { mobile });
  ...
  const handleOtpResetPassword = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await client.post('/auth/verify-otp-reset', { mobile, otp, newPassword });
        ...
        <div className="relative z-10">
          ...
          {activeTab === 'otp' && (
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center text-xs font-semibold text-gray-500 px-2 pb-2">
                <span className={otpStep >= 1 ? 'text-blue-500' : ''}>1. Mobile</span>
                <span className="text-gray-700">➔</span>
                <span className={otpStep >= 2 ? 'text-blue-500' : ''}>2. Verify OTP</span>
                <span className="text-gray-700">➔</span>
                <span className={otpStep >= 3 ? 'text-blue-500' : ''}>3. Reset Password</span>
              </div>
              ...
              {otpStep === 2 && (
                <form onSubmit={handleVerifyOtpStep} className="space-y-4">
                  ...
                  <div className="flex justify-between items-center text-xs">
                    ...
                    {otpCountdown > 0 ? (
                      <span className="text-gray-500 font-semibold">Resend in {otpCountdown}s</span>
                    ) : (
                      <button
                        type="button"
